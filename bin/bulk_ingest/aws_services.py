...
from aws.osml.data_intake.utils.logger import logger
...
class RefreshableBotoSession:
    ...
    def __init__(self):
        self.sts_arn = ServiceConfig.sts_arn
        self.session_name = "DataIntakeSession"
    ...
    def _refresh(self) -> Dict[str, str]:
        ...
        """
        session = Session(region_name=ServiceConfig.aws_region)

        if self.sts_arn:
            sts_client = session.client(service_name="sts", region_name=ServiceConfig.aws_region)
    ...
    def refreshable_session(self) -> Session:
        ...
        session.set_config_variable("region", ServiceConfig.aws_region)
...
def initialize_aws_services() -> Tuple[ServiceResource]:
    """
    Initialize AWS services required by the application.

    This function initializes S3 and SQS services,
    handling any exceptions that occur during the process.

    :return: Tuple containing the initialized service resources for s3 and sqs clients.
